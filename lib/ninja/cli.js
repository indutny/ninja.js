...
exports.run = function run(args, cliOptions, callback) {
  ...
  const b = new ninja.Builder({
    ...
    exec: (cmd, desc, callback) => {
      ...
      if (cliOptions.log)
        cliOptions.log(argv.v ? cmd : desc);

      exec(cmd, options, (err, stdout, stderr) => {
        if (err && cliOptions.error) {
          cliOptions.error('----- STDOUT -----\n' + stdout);
          cliOptions.error('----- STDERR -----\n' + stderr);
        }
        return callback(err);
      });
    },
    ...
    warning: (msg) => {
      if (cliOptions.warning)
        cliOptions.warning(msg);
    }
  });

  b.build(argv._.slice(2), intermediate, (err) => {
    if (!err && cliOptions.log)
      cliOptions.log('... done ...');
    callback(err);
  });
};
